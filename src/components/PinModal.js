    ...
    <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
      <form onSubmit={handleSubmit} style={{ background: "#fff", padding: 32, borderRadius: 12, minWidth: 320, boxShadow: "0 2px 16px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button type="button" onClick={onClose} style={{ padding: "0.5rem 1.2rem", borderRadius: 6, border: "none", background: "#eee" }}>취소</button>
          <button type="submit" style={{ padding: "0.5rem 1.2rem", borderRadius: 6, border: "none", background: "#222", color: "#fff", fontWeight: 600 }}>확인</button>
        </div>
      </form>
    </div>
  );
